...
class WandbLogger(object):
    ...
    def log_images(self, predictions):
        ...
        for idx, val in table_ref.iterrows():
            ...
            if id in predictions:
                ...
                boxes = []
                for i in range(len(prediction["resized_bboxes"])):
                    bbox = prediction["resized_bboxes"][i]
                    x0 = bbox[0]
                    y0 = bbox[1]
                    x1 = bbox[2]
                    y1 = bbox[3]
    ...
    def initialize_wandb_logger(cls, args, exp, val_dataset):
        ...
        return cls(config=vars(exp), val_dataset=val_dataset, project=args.project_name, name=args.experiment_name,**wandb_params)
